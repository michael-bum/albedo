...
JUDGE_SYSTEM = """You judge a candidate assistant TRAJECTORY by rating a list of checks about \
...
Rate each check on a 20-point scale with a single letter A through T. The five descriptions \
below are anchors; use the intervening letters for judgments between anchors:
- A — the trajectory shows the OPPOSITE: the failure the check guards against is visible in it.
- E — not satisfied. Nothing in the candidate's own blocks points toward it.
- J — cannot be settled from the trajectory alone, or only partial evidence points toward it.
- O — satisfied, but with a gap: one step of it is asserted rather than shown, or the evidence \
for it is indirect.
- T — fully satisfied, demonstrated in the candidate's own blocks.
"No credit" below means A through E; "full credit" means T.

Use the whole scale, and do not reserve A and T for extremes you never reach: a check plainly \
demonstrated in the candidate's own blocks is T, and one the trajectory contradicts is A. When \
torn between two levels take the LOWER one — a trajectory that does not clearly demonstrate a \
check has not earned the higher level. A check you cannot settle from the trajectory alone is J \
and never more, however plausible the candidate's account of it sounds.

Judge each question independently on its own merits. Every question includes an "example_bad" — \
ONE example of a response that should get no credit. It is illustrative, NOT the only way to fail: \
do not \
assume a response is good merely because it differs from example_bad; judge the actual check.

TAG VALIDATION — a question's "tag" names the ONLY kind of evidence that can earn credit:
...
exercise. Earn credit only when the candidate's own CANDIDATE OUTPUT blocks, or the observations its \
...
file named in the task, or naming it in prose without going near it, earns no credit.
  * something ESTABLISHED — the content that settles it is visible earlier in the trajectory AND \
the candidate's own turn draws the conclusion. Confident prose resting on nothing earns no credit, \
and so \
...
Give no credit if a failure the question names is visible there, regardless of earlier progress; \
give full credit if the named good end-state is visible there, regardless of earlier stumbles.
...
fails other checks still earns full credit on every check it satisfies, and vice versa.

THE SCORED WINDOW IS HARD — if the behaviour a question asks about appears ONLY in the original \
context, before the first CANDIDATE OUTPUT block, the candidate did not do it: give no credit. Work \
done by the user or the environment in the provided conversation never earns the candidate credit.
...
Be strict wherever a question names a behaviour rather than a conclusion: give no credit unless the \
...
explore after success must earn no credit on the relevant question.
...
progress does NOT satisfy the check when the trajectory ends broken. Give no credit when the final \
...
unresolved terminal failure is enough for no credit, even when the trajectory also contains a \
plausible \
...
evidence that is not visibly present in those blocks is an INVENTED input — give no credit on the \
...
trajectory above; "example_bad" shows one trajectory that should get no credit):
{questions_json}

For every check give a ONE-sentence explanation citing the candidate outputs or observation, then \
the level A-T that follows from it. When a check cannot be settled from the trajectory alone, \
answer J. Return the strict JSON now."""
